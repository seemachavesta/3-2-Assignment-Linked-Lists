...
void LinkedList::Append(Bid bid) {
    ...
    if (head == nullptr) {
        ...
        // point the current tail to the new node and update tail
        tail->next = newNode;
        tail = newNode;
    }
...
void LinkedList::Remove(string bidId) {
    ...
    while (current != nullptr && current->next != nullptr) {
        // If the next node's bidID is the one to remove
        if (current->next->bid.bidId == bidId) {
            Node* temp = current->next; // temporarily hold the node
...
Bid LinkedList::Search(string bidId) {
    ...
    // Loop through the list
    while (current != nullptr) {
        // If the current node matches the bidId
    ...
    // If no bid is found, return an empty bid
    return Bid();
}
